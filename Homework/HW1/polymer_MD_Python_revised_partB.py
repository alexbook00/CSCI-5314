...
#################################################################
def Polymer_MD_Python(epsilon_LJ_curr):
...
#####################################################################
    ...
    for step_i in range(0, steps): #molecular dynamics loop
        ...
        sizes.append(size)
        if step_i == 0:
            print(f'Starting simulation for epsilon_LJ = {epsilon_LJ_curr}')
        if step_i%10000 == 0:
            print(f'step: {step_i}')
...
def main():
    for epsilon_LJ_curr, color_curr in zip([1, .5, 0], ['red', 'blue', 'green']):
        sizes = Polymer_MD_Python(epsilon_LJ_curr)
        plt.plot(np.arange(200000), sizes, color=color_curr, label=f'\epsilon = {epsilon_LJ_curr}')
    plt.legend()
    plt.title('Folded Protein Size Over Time')
    plt.xlabel('Time (steps)')
    plt.ylabel('Size (radius of gyration)')
    plt.show()
